...
namespace BT{
    ...
    std::vector<D> Simplex(OP f,                   //target function
        ...
        { //construct the trial simplex
            ...
            std::vector<D> del( init.size(),1.0 );
            std::transform(del.begin(), del.end(), del.begin(),
                           std::bind2nd( std::divides<D>() , (double)init.size()) );//'20' is picked
        ...
        for(cnt=0; cnt<iterations; ++cnt){
            ...
            double Diff=0.0;
            for(int i=1;i<N+1;++i){
                Diff+=fabs(vf[i]-vf[i-1]);
            }



            x1=0; xn=0; xnp1=0;//find index of max, second max, min of vf.
//    cout<<" DIFF="<<Diff<<" "<<tol<<" "<<std::numeric_limits<D>::epsilon()<<" SAY ";
            if(Diff/N < tol) break;
        ...
        if(cnt==iterations){//max number of iteration achieves before tol is satisfied
            std::cout<<"WARNING !!! Iteration limit achieves, result may not be optimal"<<std::endl;
        }
        return x[x1];
    }
